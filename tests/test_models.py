...
class CovAbsTagged(Model):
    id = fields.IntField(pk=True)
    tags = fields.ManyToManyField("CovTag", related_name="tagged")

    class Meta:
        abstract = True


class CovItem(CovAbsTagged):
...
class CovAbsParented(Model):
    id = fields.IntField(pk=True)
    parent = fields.ForeignKeyField("CovParent", related_name="abs_kids")

    class Meta:
        abstract = True


class CovOverride(CovAbsParented):
    # Redeclares the inherited FK — exercises the "already present" branch.
    parent = fields.ForeignKeyField("CovParent", related_name="override_kids")
...
def test_saved_in_db_setter():
    ...
    assert obj._in_db is True


def test_override_model_keeps_redeclared_fk_relation():
    """
    GIVEN a concrete model that redeclares a FK inherited from an abstract base
    WHEN its metadata is inspected
    THEN the redeclared relation is present (inherited copy not duplicated)
    """
    assert "parent" in CovOverride._meta.relations


@pytest.mark.asyncio
async def test_abstract_m2m_relation_inherited(db):
    """
    GIVEN a many-to-many declared on an abstract base
    WHEN a concrete subclass uses it
    THEN the m2m manager works on the subclass
    """
    item = await CovItem.create(title="t")
